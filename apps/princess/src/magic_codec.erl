...
-record(magic_codec_context,{
	remain
}).

new()->
	Context = #magic_codec_context{remain = <<>>},
	{ok,Context}.

encode(Context,Data)->
	ID = Data#magic_command.id,
	Cmd = Data#magic_command.cmd,
	Content = Data#magic_command.data,
...
decode(Context,Data)->
	Remain = Context#magic_codec_context.remain,
	NewData = <<Remain/bits,Data/bits>>,
	{ok,Buff,RemainData} = princess_decode(NewData,[]),
	{ok,Buff,Context#magic_codec_context{remain = RemainData}}.
